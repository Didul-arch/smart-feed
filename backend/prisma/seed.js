const {
  ...
  Hari,
  SesiPemberianMakan,
} = require("../generated/prisma"); // Pastikan path ke client benar
...
async function main() {
  ...
  await prisma.$executeRawUnsafe(
    'ALTER SEQUENCE "Sapi_id_seq" RESTART WITH 1;'
  );
  await prisma.$executeRawUnsafe(
  ...
  const pakanDataFromJSON = [
    ...
    },
    {
      ...
      harga: 4000,
      banyakStok: 120,
      image: "bungkil_kelapa.jpg",
    },
    {
      nama: "Bungkil Kedelai",
      jenis: "Konsentrat",
      bk: 88.7,
      pk: 46.5,
      sk: 6.5,
  ...
  for (const pakan of pakanDataFromJSON) {
    const newPakan = await prisma.pakan.create({
      data: {
        ...
        harga: pakan.harga,
        image:
          pakan.image !== undefined
            ? pakan.image
            : `${pakan.nama.toLowerCase().replace(/\s+/g, "_")}.jpg`, // Default image name
        // Nutrisi
        bk: pakan.bk,
        pk: pakan.pk,
        sk: pakan.sk,
  ...
  const pakanRumputSegar =
    createdPakan.find((p) => p.nama === "Rumput Lapang") || createdPakan[0]; // Ganti dengan nama yang sesuai atau fallback
  const pakanKonsentratSuper =
    createdPakan.find((p) => p.nama === "Dedak Padi") || createdPakan[1]; // Ganti dengan nama yang sesuai atau fallback
  const pakanJeramiKering =
    createdPakan.find((p) => p.nama === "Jerami") || createdPakan[2]; // Ganti dengan nama yang sesuai atau fallback
  ...
  const sapi01 = await prisma.sapi.create({
    data: {
      ...
      jenisKelamin: "BETINA", // Tambahkan jenisKelamin
    },
  });
  const sapi02 = await prisma.sapi.create({
    data: {
      ...
      jenisKelamin: "JANTAN", // Tambahkan jenisKelamin
    },
  });
  ...
  console.log(
    `Created sapi: ${sapi01.jenis} (ID: ${sapi01.id}), ${sapi02.jenis} (ID: ${sapi02.id}), ${sapi03.jenis} (ID: ${sapi03.id})`
  );
    ...
    data: [
      {
        sapiId: sapi01.id,
        hari: Hari.SENIN,
        pagiPakanId: pakanRumputSegar.id,
        pagiWaktu: "07:00",
        sorePakanId: pakanKonsentratSuper.id,
        soreWaktu: "16:00",
      },
      {
        sapiId: sapi01.id,
      ...
      },
      {
        sapiId: sapi01.id,
        hari: Hari.RABU,
        pagiPakanId: pakanJeramiKering.id,
        pagiWaktu: "07:10",
        sorePakanId: pakanKonsentratSuper.id,
        soreWaktu: "16:10",
      },
    ...
    data: [
      {
        ...
        pagiPakanId: pakanJeramiKering.id,
        pagiWaktu: "07:30",
        sorePakanId: pakanRumputSegar.id,
        soreWaktu: "16:30",
      },
      {
        sapiId: sapi02.id,
    ...
    data: [
      {
        sapiId: sapi03.id,
        hari: Hari.SENIN,
        pagiPakanId: pakanKonsentratSuper.id,
        pagiWaktu: "08:00",
        sorePakanId: pakanRumputSegar.id,
        soreWaktu: "17:00",
      },
      {
        ...
        pagiPakanId: pakanKonsentratSuper.id,
        pagiWaktu: "08:05",
        sorePakanId: pakanJeramiKering.id,
        soreWaktu: "17:05",
      },
  ...
  await prisma.recordPemberianMakan.createMany({
    data: [
      ...
      {
        sapiId: sapi01.id,
        pakanDiberikanId: pakanRumputSegar.id,
        jumlahDiberikan: 2.5,
        tanggalPemberian: tanggalKemarin,
        sesi: SesiPemberianMakan.PAGI,
        waktuPemberianActual: new Date("2025-05-19T07:05:00.000Z"),
        kandangId: sapi01.kandangId,
      },
      {
        sapiId: sapi01.id,
        pakanDiberikanId: pakanKonsentratSuper.id,
        jumlahDiberikan: 1.0,
        tanggalPemberian: tanggalKemarin,
        sesi: SesiPemberianMakan.SORE,
        waktuPemberianActual: new Date("2025-05-19T16:10:00.000Z"),
        kandangId: sapi01.kandangId,
      },
      ...
      {
        sapiId: sapi02.id,
        pakanDiberikanId: pakanJeramiKering.id,
        jumlahDiberikan: 3.0,
        tanggalPemberian: tanggalKemarin,
        sesi: SesiPemberianMakan.PAGI,
        waktuPemberianActual: new Date("2025-05-19T07:38:00.000Z"),
        kandangId: sapi02.kandangId,
      },
      ...
      // Sapi01 (Kandang A), Hari Ini (Selasa) - hanya pagi
      {
        ...
        pakanDiberikanId: pakanRumputSegar.id,
        jumlahDiberikan: 2.5,
        tanggalPemberian: tanggalHariIni,
        sesi: SesiPemberianMakan.PAGI,
        waktuPemberianActual: new Date("2025-05-20T07:12:00.000Z"),
        kandangId: sapi01.kandangId,
      },
      ...
      // Sapi03 (Kandang B), Kemarin (Senin)
      {
        ...
        tanggalPemberian: tanggalKemarin,
        sesi: SesiPemberianMakan.PAGI,
        waktuPemberianActual: new Date("2025-05-19T08:05:00.000Z"),
        kandangId: sapi03.kandangId,
      },
      {
        sapiId: sapi03.id,
        pakanDiberikanId: pakanRumputSegar.id,
        ...
        tanggalPemberian: tanggalKemarin,
        sesi: SesiPemberianMakan.SORE,
        waktuPemberianActual: new Date("2025-05-19T17:10:00.000Z"),
        kandangId: sapi03.kandangId,
      },
    ],
  });
