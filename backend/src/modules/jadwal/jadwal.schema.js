...
const baseJadwalHarianSchema = z.object({
  pagiPakanId: z.coerce.number().int().positive().optional().nullable(),
  pagiWaktu: z
    .string()
    .regex(
      /^([01]\d|2[0-3]):([0-5]\d)$/,
      "Format waktu pagi tidak valid (HH:MM)"
    )
    .optional()
    .nullable(),
  sorePakanId: z.coerce.number().int().positive().optional().nullable(),
  soreWaktu: z
    .string()
    .regex(
      /^([01]\d|2[0-3]):([0-5]\d)$/,
      "Format waktu sore tidak valid (HH:MM)"
    )
    .optional()
    .nullable(),
});
...
const createJadwalHarianSchema = baseJadwalHarianSchema.extend({
  sapiId: z.coerce
    .number()
    .int()
    .positive({ message: "sapiId harus diisi dan berupa angka positif" }),
